...
def GetElectricSystemModel_Param_GestionSingleNode(areaConsumption,availabilityFactor,TechParameters,
                                                   price_param_df, margin_df,
                                                   empty_indexed_df,
    ...
    margin_price_df = vm_expand_grid(
        {"AREAS": AREAS, "TIMESTAMP": TIMESTAMP, "TECHNOLOGIES": TECHNOLOGIES})
    ...
    margin_price_df = margin_price_df.merge(
        ...
        margin_price=lambda x: x.margin_param * x.margin)

    # price_param_df
    price_param_df = price_param_df.merge(TechParameters[['capacity']], left_index=True, right_index=True)
    price_param_df['techno_scarc_param_on_capa'] = np.where(price_param_df['capacity'] > 0,
                                                 price_param_df['techno_scarc_param'] / price_param_df['capacity'],
                                                 0)
    ...
    obj_param_df = empty_indexed_df.merge(
        ...
        margin_price_df['margin_price'], how='left', left_index=True, right_index=True).merge(
        price_param_df['techno_scarc_param_on_capa'], how='left', left_index=True, right_index=True).assign(
        p0=lambda x: x.intercept_param + x.margin_price).assign(
        p1=lambda x: x.techno_scarc_param_on_capa)
