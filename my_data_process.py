...
def import_data(year_train, year_test, is_train, solve_model, preprocessing, drivers, window_size, step_local_linear, best_iter, is_with_bias_corr, FOLDER_OUTPUT, margin_func, fuel_func, C02_func):
    ...
    for iteration in range(nb_iteration):
        if solve_model == 'linear':
            obj_param_df = linear_create_data(empty_indexed_df, price_param_df, margin_price_df, full_fuel_price_df, full_co2_price_df, unplanned)
...
preprocessing = False
drivers = ['intercept', 'energy', 'margin', 'fuel_price', 'co2_price', 'unavail_power']
print(model)

import_data(2018, 2018, True, model, preprocessing, drivers, window_size, step_local_linear, 1, True, FOLDER_OUTPUT, id_func, id_func, id_func)

import_data(2018, 2017, False, model, preprocessing, drivers, window_size, step_local_linear, 3, True, FOLDER_OUTPUT, id_func, id_func, id_func)
