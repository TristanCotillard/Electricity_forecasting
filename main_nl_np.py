...
from update_nl_np import test_apply_model, test_create_drivers, test_prices_sim
from pyomo_nl_np import GetElectricSystemModel_Param_Interco_Storage_GestionSingleNode
from update_nl_np import create_prices_sim, create_drivers, update_price
...
def import_data(year_train, year_test, is_train, solve_model, preprocessing, window_size, step_local_linear, best_iter, is_with_bias_corr, FOLDER_OUTPUT, margin_func, fuel_func, C02_func):
    ...
    if is_train:
        prices_sim = create_prices_sim(selected_area, selected_timestamp, selected_techno)
    ...
    if not is_train:
        ...
        prices_sim = test_prices_sim(selected_area, selected_timestamp, selected_techno, drivers, FOLDER_OUTPUT)            
    ...
    for iteration in range(nb_iteration):
        ...
        obj_param_df = create_drivers(empty_indexed_df, margin_price_df, full_fuel_price_df, full_co2_price_df, prices_sim)
        ...
        all_orders_df = obj_param_df.merge(
            production_df_melted[['energy']], how='left', left_index=True, right_index=True).reorder_levels(
            ['AREAS', 'TIMESTAMP', 'TECHNOLOGIES'])
        # print(all_orders_df)
        if not is_train:
            print('apply not train')
            ...
            prices_sim['price_sim'] = all_orders_df['price_sim'] 
        # print(all_orders_df)
        ...
        merit_order_df = merit_order_df.sort_values('price_sim', ascending=False).groupby(['AREAS','TIMESTAMP']).head(1)
        ...
        if is_train:
            prices_sim = update_price(selected_techno, selected_area, selected_timestamp, for_estim_param_df, extended_for_estim_param_df, iteration == best_iter, FOLDER_OUTPUT)
        ...
        curtailment_marginal_df = curtailment_marginal_df.sort_values('price_sim', ascending=False).groupby(['AREAS','TIMESTAMP']).head(1)
...
import_data(2017, 2018, False, model, preprocessing, window_size, step_local_linear, 2, True, FOLDER_OUTPUT, id_func, id_func, id_func)
