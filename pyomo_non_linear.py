...
def GetElectricSystemModel_Param_Interco_Storage_GestionSingleNode(daPrices_df, areaConsumption,availabilityFactor,TechParameters,
    ...
    model.prices_res = Param(model.AREAS_TIMESTAMP_TECHNOLOGIES, 
                                initialize=drivers_df['price_res_sim'].squeeze().to_dict(), domain=Any)
    ...
    def ObjectiveFunction_rule(model):  # OBJ
        ...
        return (
                quicksum(
                    ((model.prices_res[area, t, tech] + model.prices_trend[area, t] + model.prices_seasonal[area, t]) * model.energy[
                        area, t, tech])
