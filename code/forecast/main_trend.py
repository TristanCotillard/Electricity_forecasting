...
def forecast_prices(year_train, year_test, is_train, solve_model, preprocessing, window_size, step_local_linear, best_iter, is_with_bias_corr, FOLDER_OUTPUT, margin_func, fuel_func, C02_func):
    ...
    fuel_prices_df = pd.read_csv(PreprocessedFolder + 'complete_fuel_prices.csv')
    fuel_prices_df = fuel_prices_df[['oil_price_trend','coal_price_trend','gas_price_trend','co2_price_trend','TIMESTAMP_d']]
    fuel_prices_df = fuel_prices_df.rename(columns={'oil_price_trend':'oil_price','coal_price_trend':'coal_price','gas_price_trend':'gas_price','co2_price_trend':'co2_price'})
    ...
    all_margin_df = pd.read_csv(PreprocessedFolder + 'complete_margin_timestamp.csv')
    
    all_margin_df = all_margin_df[['TIMESTAMP_d', 'margin_trend']]
    all_margin_df = all_margin_df.rename(columns={'margin_trend':'margin'})
    # print(timestamp_df)
    # print(all_margin_df)
    ...
    for iteration in range(nb_iteration):
        ...
        all_orders_df = obj_param_df.merge(
            ...
            order_price_trend=lambda x: x.p0 + x.p1 * x.energy).reorder_levels(
            ['AREAS', 'TIMESTAMP', 'TECHNOLOGIES'])
        
        # Add price trend and price seasonal to residue 
        all_orders_df['order_price'] = (all_orders_df['order_price_trend'] + daPrices_df['price_resid'] + daPrices_df['price_seasonal']) * mean_sd['price_sd'].values + mean_sd['price_mean'].values
        ...
        for_estim_param_df = final_merit_order_df.merge(daPrices_df['price_trend'], how = 'left', left_index = True, right_index = True)
        ...
        extended_for_estim_param_df = all_orders_df.merge(
            daPrices_df['price_trend'], how = 'left', left_index = True, right_index = True).merge(
        ...
        extended_for_estim_param_df['price_reg'] = extended_for_estim_param_df['price_factor'] * extended_for_estim_param_df['price_trend']

        # print(final_merit_order_df, extended_for_estim_param_df)
        # print(for_estim_param_df['energy'])
        # st = StandardScaler()
        # for_estim_param_df['energy'] = st.fit_transform(for_estim_param_df['energy'].values.reshape(-1, 1))
...
def truncated_log(data):
        ...
        # data[data <= 0] = 0
        return np.log( data + np.min(data) + 2)
