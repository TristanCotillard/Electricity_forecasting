...
def GetElectricSystemModel_PlaningSingleNode(areaConsumption, availabilityFactor, TechParameters, isAbstract=False):
    ...
    model.energy = Var(model.TIMESTAMP, model.TECHNOLOGIES,
    ...
    def energyCostsDef_rule(model,
                            tech):  # EQ forall tech in TECHNOLOGIES   energyCosts  = sum{t in TIMESTAMP} energyCost[tech]*energy[t,tech] / 1E6;
        ...
        def storage_rule(model, tech):  # INEQ forall t, tech
            if model.EnergyNbhourCap[tech] > 0:
                return model.EnergyNbhourCap[tech] * model.capacity[tech] >= sum(
                    model.energy[t, tech] for t in model.TIMESTAMP)
            else:
                return Constraint.Skip
    ...
    if "RampConstraintPlus" in TechParameters:
        def rampCtrPlus_rule(model, t, tech):  # INEQ forall t<
            if model.RampConstraintPlus[tech] > 0:
                return model.energy[t + 1, tech] - model.energy[t, tech] <= model.capacity[tech] * \
    ...
    if "RampConstraintPlus2" in TechParameters:
        def rampCtrPlus2_rule(model, t, tech):  # INEQ forall t<
            if model.RampConstraintPlus2[tech] > 0:
                var = (model.energy[t + 2, tech] + model.energy[t + 3, tech]) / 2 - (
                            model.energy[t + 1, tech] + model.energy[t, tech]) / 2;
...
def GetElectricSystemModel_PlaningSingleNode_withStorage(areaConsumption, availabilityFactor,
                                                         TechParameters, StorageParameters, solverpath=-1,
                                                         isAbstract=False,
                                                         solver='mosek'):
    """
    ...
    ###Storage means :
    model.storageIn = Var(model.TIMESTAMP, model.STOCK_TECHNO,
                          domain=NonNegativeReals)  ### Energy stored in a storage mean at time t
    model.storageOut = Var(model.TIMESTAMP, model.STOCK_TECHNO,
                           domain=NonNegativeReals)  ### Energy taken out of a storage mean at time t
    model.stockLevel = Var(model.TIMESTAMP, model.STOCK_TECHNO,
    ...
    def StockLevel_rule(model, t, s_tech):  # EQ forall t
        if t > 1:
            return model.stockLevel[t, s_tech] == model.stockLevel[t - 1, s_tech] * (1 - model.dissipation[s_tech]) + \
                   model.storageIn[t, s_tech] * model.efficiency_in[s_tech] - model.storageOut[t, s_tech] / \
                   model.efficiency_out[s_tech]
        else:
...
def GetElectricSystemModel_PlaningMultiNode(areaConsumption, availabilityFactor, TechParameters, ExchangeParameters,
                                            isAbstract=False):
    """
    ...
    def capacityCostsDef_rule(model, area,
        ...
        return model.capacityCost[area, tech] * len(model.TIMESTAMP) / 8760 * model.capacity[area, tech] == \
        ...
        def storageCtr_rule(model, area, tech):  # INEQ forall t, tech
            if model.EnergyNbhourCap[area, tech] > 0:
                return model.EnergyNbhourCap[area, tech] * model.capacity[area, tech] >= sum(
                    model.energy[area, t, tech] for t in model.TIMESTAMP)
            else:
                return Constraint.Skip
