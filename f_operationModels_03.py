...
def GetElectricSystemModel_Param_Interco_GestionSingleNode(areaConsumption,availabilityFactor,TechParameters,
    ...
    model.energy_import = Var(model.AREAS, model.TIMESTAMP, model.INTERCOS,domain=NonNegativeReals)  ### Energy produced by a production mean at time t
    model.energy_export = Var(model.AREAS, model.TIMESTAMP, model.INTERCOS,domain=NonNegativeReals)  ### Energy produced by a production mean at time t
    ...
    def CapacityCtr_import_rule(model,area,t,interco): #INEQ forall t, tech
    	return  model.capacity_import[area,interco] * model.availabilityFactor_import[area,t,interco] >=  model.energy_import[area,t,interco]
    ...
    def CapacityCtr_export_rule(model,area,t,interco): #INEQ forall t, tech
    	return  model.capacity_export[area,interco] * model.availabilityFactor_export[area,t,interco] >=  model.energy_export[area,t,interco]
    ...
    def energyCtr_rule(model,area,t): #INEQ forall t
    	return sum(model.energy[area,t,tech] for tech in model.TECHNOLOGIES) + sum(model.energy_import[area,t,interco] for interco in model.INTERCOS) - sum(model.energy_export[area,t,interco] for interco in model.INTERCOS) + sum(model.exchange[b,area,t]*LineEfficiency for b in model.AREAS) == model.areaConsumption[area,t]
...
def GetElectricSystemModel_Param_Storage_GestionSingleNode(areaConsumption,availabilityFactor,TechParameters,
                                                                   StorageParameters,
                                                                   obj_param_df,
                                                                   ExchangeParameters=pd.DataFrame({'empty': []}),
                                                                   isAbstract=False, LineEfficiency=1):
