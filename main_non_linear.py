...
from update_non_linear import test_apply_model, test_create_drivers, test_prices_sim
from pyomo_non_linear import GetElectricSystemModel_Param_Interco_Storage_GestionSingleNode
from update_non_linear import create_prices_sim, create_drivers, update_price
...
def import_data(year_train, year_test, is_train, solve_model, preprocessing, window_size, step_local_linear, best_iter, is_with_bias_corr, FOLDER_OUTPUT, margin_func, fuel_func, C02_func):
    ...
    if not is_train:
        ...
        area_consumption_df = pd.read_csv(InputFolder + 'r_areaConsumption_no_phes' + str(year_train) + '_' + str(area) + '.csv')
        area_consumption_df['AREAS'] = area
        timestamp_train_df = area_consumption_df.query("AREAS == 'FR'")[['TIMESTAMP','DateTime']]
    ...
    # print(timestamp_df, timestamp_train_df)

    if not is_train:
        daPrices_test_df = da_prices_df.merge(timestamp_train_df, on='TIMESTAMP_d').set_index(['AREAS','TIMESTAMP'])
        daPrices_df['price_trend'] = daPrices_test_df['price_trend']
        daPrices_df['price_seasonal'] = daPrices_test_df['price_seasonal']
    ...
    if is_train:
        prices_res_sim = create_prices_sim(selected_area, selected_timestamp, selected_techno)
    ...
    if not is_train:
        drivers = test_create_drivers(empty_indexed_df, margin_price_df, full_fuel_price_df, full_co2_price_df)
        prices_res_sim = test_prices_sim(selected_area, selected_timestamp, selected_techno, drivers, FOLDER_OUTPUT)            
    ...
    for iteration in range(nb_iteration):
        ...
        obj_param_df = create_drivers(empty_indexed_df, margin_price_df, full_fuel_price_df, full_co2_price_df, prices_res_sim)
        ...
        all_orders_df = obj_param_df.merge(
            ...
            order_price_residue=lambda x:x.price_res_sim).reorder_levels(
            ['AREAS', 'TIMESTAMP', 'TECHNOLOGIES'])
        
        if not is_train:
            drivers = ['energy', 'margin', 'fuel_price', 'co2_price']
            all_orders_df = test_apply_model(all_orders_df, drivers, selected_area, selected_techno, FOLDER_OUTPUT)

        # Add price trend and price seasonal to residue 
        all_orders_df['order_price'] = all_orders_df['order_price_residue'] + daPrices_df['price_trend'] + daPrices_df['price_seasonal']
        ...
        if is_train:
            prices_res_sim = update_price(selected_techno, selected_area, selected_timestamp, for_estim_param_df, extended_for_estim_param_df, iteration == best_iter, FOLDER_OUTPUT)
...
# import_data(2017, 2017, True, model, preprocessing, window_size, step_local_linear, 4, True, FOLDER_OUTPUT, id_func, id_func, id_func)

import_data(2017, 2018, False, model, preprocessing, window_size, step_local_linear, 0, True, FOLDER_OUTPUT, id_func, id_func, id_func)
